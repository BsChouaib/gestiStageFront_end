...
@Component({
  selector: 'app-delete-demand-dialog',
  templateUrl: './delete-demand-dialog.component.html',
  styleUrls: ['./delete-demand-dialog.component.sass'],
  standalone: true,
...
export class DeleteDemandDialogComponent implements OnInit, OnDestroy {
  ...
  constructor(
    public dialogRef: MatDialogRef<DeleteDemandDialogComponent>,
  ...
  submit() {
    ...
    let id =this.demand.demandId
    this.demandService.deleteDemand(id).subscribe({
      next:(res)=>{
        this.toast.clear(loadingToast.toastId);

        this.toast.success('Demand deleted Successfully', 'Success', {
          progressBar: true,
          closeButton: true,
        });
      },
      ...
      error:(err)=>{
          console.log(err)
          this.toast.clear(loadingToast.toastId);
          this.toast.error('Delete demand Failed', 'Error', {
           progressBar: true,
           closeButton: true,
         });
      }
