...
export class AuthUtils {
    ...
    /**
     * Get is_profile_filled value
     *
     * @param token
     * @private
     */
    private static _get_is_profile_filled(token: string): boolean | null {
        // Get the decoded token
        const decodedToken = this._decodeToken(token);

        // Return if the decodedToken doesn't have an 'exp' field
        if ( !decodedToken.hasOwnProperty('is_profile_filled') ) {
            return null;
        }
        return decodedToken.is_profile_filled
    }
     ...
     * @param offsetSeconds
     */
    static isProfileFilled(token: string): boolean {
        // Return if there is no token
        if ( !token || token === '' ) {
            return true;
        }

        return this._get_is_profile_filled(token);
    }
    ...
    //     try {
    //         // Replace 'yourSecretKey' with the actual secret key used for signing the JWT token
